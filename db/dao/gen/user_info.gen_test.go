...
func Test_userInfoDBModelQuery(t *testing.T) {
	...
	primaryKey := field.NewString(userInfoDBModel.TableName(), clause.PrimaryKey)
	...
	_, ok := userInfoDBModel.GetFieldByName("")
	if ok {
		t.Error("GetFieldByName(\"\") from userInfoDBModel success")
	}

	err = _do.Create(&model.UserInfoDBModel{})
	if err != nil {
		t.Error("create item in table <user_info> fail:", err)
	}

	err = _do.Save(&model.UserInfoDBModel{})
	if err != nil {
		t.Error("create item in table <user_info> fail:", err)
	}

	err = _do.CreateInBatches([]*model.UserInfoDBModel{{}, {}}, 10)
	if err != nil {
		t.Error("create item in table <user_info> fail:", err)
	}

	_, err = _do.Select(userInfoDBModel.ALL).Take()
	if err != nil {
	...
	err = _do.Where(primaryKey.IsNotNull()).FindInBatches(&[]*model.UserInfoDBModel{}, 10, func(tx gen.Dao, batch int) error { return nil })
	if err != nil {
		t.Error("FindInBatches() on table <user_info> fail:", err)
	}

	_, err = _do.Select(userInfoDBModel.ALL).Where(primaryKey.IsNotNull()).Order(primaryKey.Desc()).Find()
	if err != nil {
	...
	_, err = _do.Select(userInfoDBModel.ALL).Omit(primaryKey).Take()
	if err != nil {
	...
	_, err = _do.ScanByPage(&model.UserInfoDBModel{}, 0, 1)
	if err != nil {
